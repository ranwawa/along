...
function findThread(owner: string, repo: string, issueNumber: number): ThreadRow | undefined {
...
function makeStmt(sql: string) {
  ...
  return {
    get: (...args: any[]) => {
      ...
      if (normalized.includes("FROM plan_revisions") && normalized.includes("AND plan_id = ?")) {
      ...
      if (normalized.includes("FROM plan_revisions") && normalized.includes("AND version = ?")) {
      ...
      if (normalized.includes("FROM plan_revisions") && normalized.includes("AND comment_id = ?")) {
    ...
    run: (...args: any[]) => {
      ...
      if (normalized.startsWith("INSERT INTO comment_mirror")) {
        ...
        const existing = state.comments.find((row) => row.comment_id === commentId);
        const next = {
          owner,
          repo,
          issue_number: issueNumber,
      ...
      if (normalized.startsWith("UPDATE plan_revisions SET status = ? WHERE owner = ? AND repo = ? AND issue_number = ? AND plan_id != ?")) {
      ...
      if (normalized.startsWith("UPDATE planning_threads SET version = version + 1, current_plan_id = ?, approved_plan_id = NULL, updated_at = ?")) {
      ...
      if (normalized.startsWith("UPDATE planning_threads SET version = version + 1, open_round_id = NULL, last_processed_comment_id = ?, updated_at = ?")) {
        const [lastProcessedCommentId, updatedAt, owner, repo, issueNumber] = args;
        const row = findThread(owner, repo, issueNumber);
        if (!row) return { changes: 0 };
        row.version += 1;
      ...
      if (normalized.startsWith("UPDATE planning_threads SET version = version + 1, open_round_id = ?, approved_plan_id = NULL, last_processed_comment_id = ?, updated_at = ?")) {
        const [roundId, lastProcessedCommentId, updatedAt, owner, repo, issueNumber] = args;
        const row = findThread(owner, repo, issueNumber);
        if (!row) return { changes: 0 };
        row.version += 1;
      ...
      if (normalized.startsWith("UPDATE planning_threads SET version = version + 1, approved_plan_id = ?, updated_at = ?")) {
        const [planId, updatedAt, owner, repo, issueNumber] = args;
        const row = findThread(owner, repo, issueNumber);
        if (!row) return { changes: 0 };
        row.version += 1;
      ...
      if (normalized.startsWith("UPDATE discussion_rounds SET status = ?, resolution = ?, produced_plan_id = ?, resolved_at = ?")) {
        const [status, resolution, producedPlanId, resolvedAt, owner, repo, issueNumber, roundId] = args;
