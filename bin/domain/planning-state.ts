...
export interface PlanningContextPayload {
  ...
  openRound: (DiscussionRoundRecord & { comments: CommentMirrorRecord[] }) | null;
...
function parsePlanSections(body: string): PlanSections {
  ...
  const summary =
    firstHeading >= 0
      ? body.slice(0, firstHeading).replace(/^<!--[\s\S]*?-->\s*/m, "").trim()
...
export function getCurrentPlanRevision(
  ...
  return getPlanRevision(owner, repo, issueNumber, threadRes.data.currentPlanId);
}
...
function createOpenRoundFromComments(
  ...
  try {
    ...
    const snapshotLastSeenCommentId = snapshotCommentIds[snapshotCommentIds.length - 1];
