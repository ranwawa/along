...
async function readRepoInfo(repoPath: string): Promise<{ owner: string; repo: string } | null> {
  try {
...
export async function buildRegistry(workspaceDirs: string[]): Promise<Result<WorkspaceRegistry>> {
  ...
  async function scan() {
    ...
    for (const dir of workspaceDirs) {
      ...
      for (const [key, val] of found) {
        if (registry.has(key)) {
          logger.warn(`仓库 ${key} 在多个位置发现，使用: ${registry.get(key)}，忽略: ${val}`);
          continue;
        }
        registry.set(key, val);
      }
    }
    logger.info(`工作区扫描完成，发现 ${registry.size} 个仓库: ${[...registry.keys()].join(", ")}`);
  }
