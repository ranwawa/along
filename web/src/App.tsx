...
import type { SessionDiagnostic, DashboardSession, StatusCounts, UnifiedLogEntry, ConversationFileInfo, ConversationMessage } from './types';
import './index.css';
...
function App() {
  ...
  const [conversationFiles, setConversationFiles] = useState<ConversationFileInfo[]>([]);
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      try {
        const res = await fetch(`/api/logs/conversation/files?${params.toString()}`);
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    const sse = new EventSource(`/api/logs/conversation/stream?${params.toString()}`);
  ...
  const renderConversationMessage = (msg: ConversationMessage, i: number) => {
    if (msg.type === 'assistant') {
      ...
      return (
        <div key={`conv-${i}`} className="p-2 rounded bg-blue-500/10 border border-blue-500/20 whitespace-pre-wrap break-all">
          <span className="text-blue-300 font-semibold text-xs mr-2">[assistant]</span>
          <span>{texts.join('\n')}</span>
        </div>
      );
    }
    if (msg.type === 'user') {
      const texts = (msg.message?.content || [])
        ...
        .map(c => c.text);
      return (
        <div key={`conv-${i}`} className="p-2 rounded bg-emerald-500/10 border border-emerald-500/20 whitespace-pre-wrap break-all">
          <span className="text-emerald-300 font-semibold text-xs mr-2">[user]</span>
    ...
    if (msg.type === 'result') {
      ...
      return (
        <div key={`conv-${i}`} className="p-2 rounded bg-amber-500/10 border border-amber-500/20">
          <span className="text-amber-300 font-semibold text-xs mr-2">[result]</span>
          <span>{msg.subtype || 'done'} {turns ? `(${turns} turns` : ''}{cost ? `, ${cost})` : turns ? ')' : ''}</span>
        </div>
      );
    }
    if (msg.type === 'tool_use_summary') {
      return (
        <div key={`conv-${i}`} className="p-2 rounded bg-purple-500/10 border border-purple-500/20">
          <span className="text-purple-300 font-semibold text-xs mr-2">[tool]</span>
          <span className="text-gray-300">{msg.tool_name || 'unknown'}</span>
          {msg.tool_input && <span className="text-text-muted ml-2 text-xs">{String(msg.tool_input).slice(0, 200)}</span>}
        </div>
      );
    }
    return (
      <div key={`conv-${i}`} className="p-1.5 rounded bg-white/5 text-text-muted text-xs">
        <span className="font-semibold mr-2">[{msg.type}{msg.subtype ? `:${msg.subtype}` : ''}]</span>
        ...
        <div className="flex-1 min-h-0 overflow-auto flex flex-col gap-1.5">
          {convLoading
            ...
            : conversationMessages.length === 0
              ...
              : conversationMessages.map((msg, i) => renderConversationMessage(msg, i))
          }
        </div>
      </div>
    );
