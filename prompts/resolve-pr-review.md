...
**获取 PR 详情与评论**（执行安全校验）：
```bash
./.along/bin/setup.ts pr-reviews $1
```
...
执行状态更新：
```bash
./.along/bin/setup.ts issue-status $1 running "已获取 PR 详情" --step "Review 代码变更"
```
...
执行状态更新：
```bash
./.along/bin/setup.ts issue-status $1 running "已完成代码 Review" --step "运行测试验证"
```
...
2. **决策与执行**：
   - **认可（Agree）**：
     ...
     b. **修改完成后，使用 pr-reply 脚本回复评论**：
        ```bash
        ./.along/bin/setup.ts pr-reply $1 $comment_id "已按建议修正"
        ```
   - **异议（Disagree/Discuss）**：
...
1. **原子化提交并推送**：
   ...
   - ```bash
     ./.along/bin/setup.ts commit-push --message "fix(scope): 按评审意见修复问题" --files 文件1
     ```

2. **更新状态为完成**：
   - ```bash
     ./.along/bin/setup.ts pr-status $1 completed --message="已按评审意见完成修改"
     ```
...
1. **更新状态为 error**：
   ```bash
   ./.along/bin/setup.ts pr-status $1 error --message="终止原因"
   ```
...
2. **可选清理**：
   ...
   # 保留现场排查
   # 或清理：
   ./.along/bin/setup.ts pr-cleanup $1
   ```

### 强制重新开始
```bash
# 1. 强制清理
./.along/bin/setup.ts pr-cleanup $1 --force
