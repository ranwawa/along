...
it('当创建 Task 指定默认分支模式时，期望 workspaceMode 传给创建层', async () => {
  const response = await handleTaskApiRequest(
    jsonRequest('/api/tasks', {
      body: '直接在默认分支修改并推送。',
...
it('当创建 Task 指定非法执行位置时，期望返回 400', async () => {
  const response = await handleTaskApiRequest(
    jsonRequest('/api/tasks', {
      body: '创建非法执行位置 planning task。',
      workspaceMode: 'main',
    }),
    new URL('http://localhost/api/tasks'),
    {
  ...
  expect(response.status).toBe(400);
  expect(payload.error).toBe('workspaceMode 必须是 worktree 或 default_branch');
  expect(planningMocks.createPlanningTask).not.toHaveBeenCalled();
});
