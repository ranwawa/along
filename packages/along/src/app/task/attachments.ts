...
const FORMAT_BYTES_PRECISION_FACTOR = 10;
const MAX_ATTACHMENT_COUNT = 6;
const MAX_SINGLE_ATTACHMENT_MIB = 10;
const MAX_TOTAL_ATTACHMENT_MIB = 30;
const ATTACHMENT_ID_LENGTH = 12;
const PNG_SIGNATURE = Buffer.from('\x89PNG', 'binary');
const JPEG_SIGNATURE = Buffer.from('\xff\xd8\xff', 'binary');
const GIF_SIGNATURE = Buffer.from('GIF8', 'ascii');
const WEBP_HEADER_BYTE_LENGTH = 'RIFFxxxxWEBP'.length;
const WEBP_FORMAT_OFFSET = 'RIFFxxxx'.length;

export const TASK_ATTACHMENT_LIMITS = {
  maxCount: MAX_ATTACHMENT_COUNT,
  maxSingleBytes: MAX_SINGLE_ATTACHMENT_MIB * BYTES_PER_MIB,
  maxTotalBytes: MAX_TOTAL_ATTACHMENT_MIB * BYTES_PER_MIB,
} as const;
...
function formatBytes(bytes: number): string {
  return `${
    Math.round((bytes / BYTES_PER_MIB) * FORMAT_BYTES_PRECISION_FACTOR) /
    FORMAT_BYTES_PRECISION_FACTOR
  }MB`;
}
...
function generateAttachmentId(): string {
  return `att_${crypto
    .randomUUID()
    .replace(/-/g, '')
    .slice(0, ATTACHMENT_ID_LENGTH)}`;
}

function hasBytePrefix(bytes: Uint8Array, signature: Uint8Array): boolean {
  return signature.every((byte, index) => bytes[index] === byte);
}

function detectImageMimeType(bytes: Uint8Array): string | undefined {
  if (hasBytePrefix(bytes, PNG_SIGNATURE)) {
    return 'image/png';
  }
  if (hasBytePrefix(bytes, JPEG_SIGNATURE)) {
    return 'image/jpeg';
  }
  if (hasBytePrefix(bytes, GIF_SIGNATURE)) {
    return 'image/gif';
  }
  const header = Buffer.from(bytes.slice(0, WEBP_HEADER_BYTE_LENGTH)).toString(
    'ascii',
  );
  if (
    header.startsWith('RIFF') &&
    header.slice(WEBP_FORMAT_OFFSET, WEBP_HEADER_BYTE_LENGTH) === 'WEBP'
  ) {
    return 'image/webp';
  }
  return undefined;
