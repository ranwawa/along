...
export async function ensureWebhookSecret(opts: {
  secret?: string;
...
export function ensureWorkspaces(): Result<string[]> {
  ...
  console.log('每个路径可以是:');
  console.log(
    `  - 一个 git 仓库的根目录（如 ${chalk.cyan('/Users/you/projects/my-repo')}）`,
  );
  console.log(
    `  - 一个包含多个 git 仓库的父目录（如 ${chalk.cyan('/Users/you/projects')}）`,
  );
  console.log('');
...
export async function printGitHubAppGuide() {
  ...
  console.log(
    chalk.bold.cyan('║        Along — GitHub App 配置指引               ║'),
  );
  console.log(
    chalk.bold.cyan('╚══════════════════════════════════════════════════╝'),
  );
  console.log('');
  console.log(
    'Along 通过 GitHub App webhook 直接接收仓库事件，无需 GitHub Actions 中转。',
  );
  ...
  console.log(
    `  ${chalk.yellow('GitHub App name')}                  - 任意名称，如 ${chalk.cyan('along-webhook')}`,
  );
  console.log(
    `  ${chalk.yellow('Description')}                      - ${chalk.dim('(可选)')} 如 ${chalk.cyan('Along 自动化 webhook 接收器')}`,
  );
  console.log(
    `  ${chalk.yellow('Homepage URL')}                     - 任意 URL，如 ${chalk.cyan('https://github.com')}`,
  );
  console.log('');
  console.log(chalk.bold('  [用户授权 — 全部跳过]'));
  console.log(
    `  ${chalk.yellow('Callback URL')}                     - ${chalk.dim('留空')}`,
  );
  console.log(
    `  ${chalk.yellow('Expire user authorization tokens')} - ${chalk.dim('默认即可')}`,
  );
  console.log(
    `  ${chalk.yellow('Request user authorization')}       - ${chalk.dim('不勾选')}`,
  );
  console.log(
    `  ${chalk.yellow('Enable Device Flow')}               - ${chalk.dim('不勾选')}`,
  );
  console.log('');
  console.log(chalk.bold('  [安装后 — 全部跳过]'));
  console.log(
    `  ${chalk.yellow('Setup URL')}                        - ${chalk.dim('留空')}`,
  );
  console.log(
    `  ${chalk.yellow('Redirect on update')}               - ${chalk.dim('不勾选')}`,
  );
  console.log('');
  ...
  console.log('');
  console.log(
    `  ${chalk.yellow('Active')}                           - ${chalk.green('✓ 勾选')}`,
  );
  console.log(
    `  ${chalk.yellow('Webhook URL')}                      - ${chalk.cyan('<你的公网地址>/webhook')}`,
  );
  console.log('');
  console.log('    需要通过隧道工具将本地端口暴露到公网:');
  console.log(
    `    ${chalk.cyan('ngrok http 9876')}                    → 复制生成的 https://xxx.ngrok-free.app`,
  );
  console.log(
    `    ${chalk.cyan('cloudflared tunnel --url http://localhost:9876')}  → 复制生成的 https://xxx.trycloudflare.com`,
  );
  console.log(
    `  ${chalk.yellow('Webhook secret')}                   - 用以下命令生成:`,
  );
  console.log(
    `                                     ${chalk.cyan('openssl rand -hex 32')}`,
  );
  console.log(
    `                                     ${chalk.dim('记下这个值，启动 webhook-server 时要用')}`,
  );
  console.log('');

  console.log(
    chalk.bold.cyan('─── Step 3: 设置权限 (Repository permissions) ───'),
  );
  console.log('');
  console.log('  只需设置以下 4 项，其余全部保持 No access:');
  console.log('');
  console.log(
    `  ${chalk.yellow('Issues')}           → ${chalk.cyan('Read & write')}   读取 Issue 内容、管理标签`,
  );
  console.log(
    `  ${chalk.yellow('Pull requests')}    → ${chalk.cyan('Read-only')}      读取 PR 信息`,
  );
  console.log(
    `  ${chalk.yellow('Checks')}           → ${chalk.cyan('Read-only')}      读取 CI 运行结果`,
  );
  console.log(
    `  ${chalk.yellow('Metadata')}         → ${chalk.cyan('Read-only')}      ${chalk.dim('(默认已勾选，不可取消)')}`,
  );
  console.log('');
  console.log(
    chalk.dim(
      '  Organization permissions / Account permissions → 全部保持默认，不需要改',
    ),
  );
  console.log('');

  console.log(
    chalk.bold.cyan('─── Step 4: 订阅事件 (Subscribe to events) ───'),
  );
  console.log('');
  console.log('  勾选以下 5 个事件（设置权限后才会出现对应选项）:');
  console.log('');
  console.log(
    `  ${chalk.green('☑')} Issues                    Issue 创建、标签变更等`,
  );
  console.log(
    `  ${chalk.green('☑')} Issue comments             Issue 评论（/approve 等指令）`,
  );
  console.log(
    `  ${chalk.green('☑')} Pull request              PR 创建、代码更新等`,
  );
  ...
  console.log('');
  console.log(
    `  ${chalk.green('◉')} Only on this account      ${chalk.dim('只在自己账号下使用')}`,
  );
  console.log(
    `  ${chalk.dim('○')} Any account                ${chalk.dim('不需要')}`,
  );
  console.log('');
  ...
  if (repoInfo.success) {
    ...
    console.log(
      `  3. 选择 ${chalk.cyan('Only select repositories')} → 选中 ${chalk.cyan(`${owner}/${repo}`)}`,
    );
  } else {
    console.log(
      `  3. 选择 ${chalk.cyan('Only select repositories')} → 选中目标仓库`,
    );
  }
  ...
  console.log('');
  console.log(
    `  ${chalk.cyan('along webhook-server --port 9876 --secret <你的-webhook-secret>')}`,
  );
  console.log('');
  console.log('  验证是否正常工作:');
  console.log(`  1. 在目标仓库创建一个测试 Issue`);
  console.log(
    `  2. 观察 webhook-server 终端是否输出 ${chalk.green('收到事件: issues.opened')}`,
  );
  console.log('');
  ...
  console.log('');
  console.log(
    `  ${chalk.yellow('issues.opened')}              → triageIssue + agent         ${chalk.dim('新 Issue 自动分类处理')}`,
  );
  console.log(
    `  ${chalk.yellow('issue_comment (/approve)')}   → launchIssueAgent(impl)      ${chalk.dim('Phase 2 自动启动')}`,
  );
  console.log(
    `  ${chalk.yellow('issue_comment (/reject)')}    → session.failed               ${chalk.dim('方案被拒绝')}`,
  );
  console.log(
    `  ${chalk.yellow('pull_request.opened')}        → reviewPr()                  ${chalk.dim('自动代码审查')}`,
  );
  console.log(
    `  ${chalk.yellow('pull_request.synchronize')}   → reviewPr()                  ${chalk.dim('代码更新后重新审查')}`,
  );
  console.log(
    `  ${chalk.yellow('pull_request_review')}        → resolveReview()             ${chalk.dim('处理 Review 反馈')}`,
  );
  console.log(
    `  ${chalk.yellow('check_run.completed')}        → resolveCi()                 ${chalk.dim('CI 失败自动修复')}`,
  );
  console.log('');
}
