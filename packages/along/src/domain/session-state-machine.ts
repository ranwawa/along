...
export type SessionStateEvent =
  ...
  | {
      type: (typeof EVENT)['AGENT_EXITED_FAILURE'];
      message: string;
      exitCode?: number;
      crashLog?: string;
    }
  ...
  | {
      type: (typeof EVENT)['MANUAL_STATUS_UPDATE'];
      lifecycle: SessionLifecycle;
      message?: string;
      step?: SessionStep;
    };
...
export function isActiveSessionLifecycle(
  lifecycle?: SessionLifecycle,
): boolean {
...
function nextStatePatch(
  ...
  const nextLifecycle =
    updates.lifecycle ?? current?.lifecycle ?? LIFECYCLE.RUNNING;
  ...
  return {
    ...
    patch: {
      ...
      error:
        updates.error === null ? undefined : (updates.error ?? current?.error),
  ...
  switch (event.type) {
    ...
    case EVENT.BRANCH_PREPARED:
      return nextStatePatch(current, {
        ...
        step: STEP.EDIT_CODE,
        message: event.branchName
          ? `已创建语义化分支 ${event.branchName}`
          : '已创建语义化分支',
    ...
    case EVENT.APPROVED:
      if (
        ...
        current?.lifecycle === LIFECYCLE.WAITING_HUMAN
      ) {
        return nextStatePatch(current, {
          lifecycle: LIFECYCLE.RUNNING,
          phase: PHASE.IMPLEMENTATION,
