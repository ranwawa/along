...
import { type GitHubIssue, get_gh_client } from '../integration/github-client';
...
export class Issue {
  ...
  checkHealth(options?: { skipWipCheck?: boolean }): Result<null> {
    ...
    if (this.data.state !== 'open') {
      return failure(
        `Issue#${this.taskNo}状态为${this.data.state}，无法处理（需为 open）`,
      );
    }
    ...
    if (!options?.skipWipCheck) {
      if (labels.some((l: string) => l === LIFECYCLE.RUNNING)) {
        return failure(
          `Issue#${this.taskNo}带有${LIFECYCLE.RUNNING}标签（agent正在运行），已主动阻断执行`,
        );
      }
    }
