...
export function generateSessionDiagnostic(
  ...
  return {
    ...
    phase: session.phase,
    exitCode: session.error?.code?.startsWith('EXIT_')
      ? Number(session.error.code.replace('EXIT_', ''))
      : undefined,
...
export function writeSessionDiagnostic(
  paths: SessionPathManager,
...
export function readSessionDiagnostic(
  paths: SessionPathManager,
