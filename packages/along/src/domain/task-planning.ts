...
export type { Lifecycle, WorkflowKind };
export { LIFECYCLE, WORKFLOW_KIND };
...
function normalizeLifecycle(value: string | null | undefined): Lifecycle {
  if (
...
function deriveTaskStatusFromWorkflow(
  ...
  if (workflow.lifecycle === LIFECYCLE.DONE) {
    return TASK_STATUS.CLOSED;
  }
  if (workflow.lifecycle === LIFECYCLE.FAILED) {
...
function getCurrentTaskFlowStageId(input: {
  ...
  if (failedStage?.stage === TASK_AGENT_STAGE.EXEC) {
    return 'exec';
  }
  ...
  if (execStage?.status === AGENT_RUN_STATUS.RUNNING) {
    return 'exec';
  }
...
function buildTaskFlowActions(input: {
  ...
  const execStepsApproved = approvedPlan
    ? areExecStepsApproved(input, approvedPlan)
    : false;
  const needsExecStepsConfirmation = Boolean(execSteps && !execStepsApproved);
  ...
  return [
    ...
    buildTaskFlowAction({
      id: needsExecStepsConfirmation ? 'confirm_exec_steps' : 'start_exec',
      label: needsExecStepsConfirmation
        ? '确认步骤并开始实现'
        : execStepsApproved
          ? '开始实现'
          : '产出实施步骤',
      description: needsExecStepsConfirmation
...
function buildRetryFailedStageAction(
...
function ensureTaskCanRecordAgentResult(taskId: string): Result<void> {
  ...
  try {
    ...
    if (normalizeLifecycle(row.lifecycle) === LIFECYCLE.DONE) {
...
export function publishPlanningUpdate(
  ...
  try {
    ...
    const txn = db.transaction(() => {
      ...
      } else if (snapshot.task.currentWorkflowKind === WORKFLOW_KIND.PLAN) {
        db.prepare(
          `
            UPDATE task_threads
            SET status = ?, updated_at = ?
...
export function recordTaskExecFailure(
  ...
  const stateRes = transitionTaskWorkflow({
    taskId: input.taskId,
...
export function approveTaskExecSteps(
  ...
  const existingApproval = findExecStepsApprovalArtifact(
    snapshot,
    approvedPlan,
  );
...
export function transitionTaskWorkflow(input: {
  ...
  try {
    ...
    const currentState: WorkflowRuntimeState = {
      ...
      currentWorkflowKind:
        (taskRow.current_workflow_kind as WorkflowKind) || WORKFLOW_KIND.PLAN,
      workflowState:
    ...
    const txn = dbRes.data.transaction(() => {
      dbRes.data
        ...
        .run(next.lifecycle, next.currentWorkflowKind, now, input.taskId);
      if (threadStatus) {
        dbRes.data
          .prepare(
            `
          ...
          .run(threadStatus, now, input.taskId, input.taskId);
      }
    });
    txn();
    return success(undefined);
