...
import type {
  LogCategory,
  UnifiedLogEntry,
} from '../../adapters/logging/log-types';
import { config } from '../../core/config';

const TIMESTAMP_PART_COUNT = 6;
...
export function listConversationFiles(
  ...
  return fs
    ...
    .map((filename) => {
      ...
      const tsRaw = parts.slice(0, TIMESTAMP_PART_COUNT).join('-');
      const rest = parts.slice(TIMESTAMP_PART_COUNT);
