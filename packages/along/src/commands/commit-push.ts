...
async function updateStatusAfterPush(
  branchName: string,
): Promise<{ issueNumber: string; owner: string; repo: string } | null> {
  ...
  return {
    issueNumber: String(found.issueNumber),
    owner: found.owner,
    repo: found.repo,
  };
}
  ...
  try {
    ...
    if (!status) {
      ...
      for (const commit of commits) {
        ...
        const tempMsgFile = path.join(
          os.tmpdir(),
          `along-commit-msg-${Date.now()}-${crypto.randomBytes(4).toString('hex')}.txt`,
        );
    ...
    if (sessionInfo) {
      ...
      const paths = new SessionPathManager(
        owner,
        repo,
        Number(sessionInfo.issueNumber),
      );
      const session = new SessionManager(
        owner,
        repo,
      ...
      completeTodoStep(
        paths,
        4,
        `已提交并推送 ${commits.length} 个 commit`,
        outputFile,
      );
    }
  } catch (error: any) {
      ...
      if (currentBranchRes.success) {
        ...
        if (foundRes.success && foundRes.data) {
          const session = new SessionManager(
            foundRes.data.owner,
            foundRes.data.repo,
            foundRes.data.issueNumber,
          );
          session.log(
            `commit-push 失败: ${error.message}\n${error.stack || ''}`,
            'error',
          );
        }
      }
