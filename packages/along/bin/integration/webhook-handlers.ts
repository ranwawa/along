...
import type { GitHubCheckRun, GitHubReviewComment } from './github-client';
 ...
 */
function findIssueByPr(
  owner: string,
  repo: string,
  prNumber: number,
): { issueNumber: number; statusData: any; paths: SessionPathManager } | null {
...
export function reviewPr(
  ...
  return withIssueLock(owner, repo, found.issueNumber, () =>
    doReviewPr(owner, repo, prNumber),
  );
}

async function doReviewPr(
  owner: string,
  repo: string,
  prNumber: number,
): Promise<void> {
  ...
  await withIssueContext(owner, repo, issueNumber, async () => {
    ...
    const agentRes = await execAgent(
      ...
      'review-pr-diff',
      async (pid) => {
        await session.updateStep(
          statusData.step || STEP.AWAIT_MERGE,
          undefined,
          statusData.phase || PHASE.STABILIZATION,
          undefined,
          pid,
        );
      },
    ...
    if (agentRes.data.exitCode !== 0) {
      const nativeMsg = agentRes.data.nativeError
        ? ` [${agentRes.data.nativeError.message}]`
        : '';
      logger.error(
        `Reviewer Agent 异常退出 (退出码: ${agentRes.data.exitCode})${nativeMsg}`,
      );
...
export function resolveReview(
  ...
  return withIssueLock(owner, repo, found.issueNumber, () =>
    doResolveReview(owner, repo, prNumber),
  );
}

async function doResolveReview(
  owner: string,
  repo: string,
  prNumber: number,
): Promise<void> {
  ...
  await withIssueContext(owner, repo, issueNumber, async () => {
    ...
    await session.transition({
      type: EVENT.REVIEW_FIX_STARTED,
      commentCount: unresolvedComments.length,
    });

    session.logEvent('agent-launched', {
      trigger: 'webhook-review',
      commentCount: unresolvedComments.length,
    });
    const agentRes = await execAgent(
      statusData.worktreePath,
      ...
      async (pid) => {
        await session.updateStep(
          ...
          PHASE.STABILIZATION,
          {
            reviewCommentCount: unresolvedComments.length,
    ...
    if (agentRes.data.exitCode !== 0) {
      const nativeMsg = agentRes.data.nativeError
        ...
        : '';
      logger.error(
        `Agent 异常退出 (退出码: ${agentRes.data.exitCode})${nativeMsg}`,
      );
      await session.markAsCrashed(
        `Agent 异常退出 (退出码: ${agentRes.data.exitCode})${nativeMsg}`,
      );
...
export function resolveCi(
  ...
  return withIssueLock(owner, repo, found.issueNumber, () =>
    doResolveCi(owner, repo, prNumber),
  );
}

async function doResolveCi(
  owner: string,
  repo: string,
  prNumber: number,
): Promise<void> {
 ...
 */
export async function cleanupIssueSession(
  owner: string,
  repo: string,
  issueNumber: number,
  repoPath?: string,
  ...
  return cleanupIssueAssets(
    String(issueNumber),
    { force: true, reason: 'webhook-cleanup', silent: true },
    owner,
    repo,
    repoPath,
  );
